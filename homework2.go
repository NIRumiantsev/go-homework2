...
func generateUsers(count int, users chan User) {
	for i := 0; i < count; i++ {
		var logsNumber = rand.Intn(1000)

		logsChannel := make(chan logItem)
		go generateLogs(logsNumber, logsChannel)

		logs := make([]logItem, logsNumber)
		for currentLog := range logsChannel {
...
func generateLogs(count int, logs chan logItem) {
	for i := 0; i < count; i++ {
		logs <- logItem{
